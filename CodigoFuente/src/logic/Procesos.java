...
public class Procesos implements Runnable {
	...
	private Thread threadDesbloqueados;
	...
	public Procesos(ColaProcesos procesosListo) {
		...
		this.procesadores.add(new Ejecucion("Procesador1", this.quantum));
	...
	public Procesos(ColaProcesos procesosListo, int numeroProcesadores, int quantum) {
		...
		for (int i = 0; i < this.numeroProcesadores; i++) {
			this.procesadores.add(new Ejecucion(("Procesador " + (i + 1)), this.quantum));
		}
	 ...
	 * @return
	 */
	public Ejecucion bloquear(String idProceso) {
		for (int i = 0; i < this.procesadores.size(); i++) {
			Ejecucion ejecucion = this.procesadores.get(i);
			if (ejecucion.getProceso().getNombre().equals(idProceso)) {
				return this.procesadores.get(i);
			}
		}
		this.refrescar = true;
		return null;
	...
	public boolean isFinalizado() {
		return this.procesosListo.isVacia() && this.procesosBloqueado.isVacia()
				&& (this.procesosTerminado.getTamano() == this.numProcesos);
	}
	...
	private void verificarProcesadores() {
		...
		for (int i = 0; i < this.numeroProcesadores; i++) {
			if (this.procesadores.get(i).getProceso_2() != null) {
				if (this.procesadores.get(i).getProceso_2().getEstado().equals(Estado.TERMINADO)) {
	...
	private void ejecutar() {
		...
		this.thread.start();
		this.threadDesbloqueados = new Thread(new Runnable() {
			@Override
			public void run() {
				while (true) {
					// System.out.println("verificando desbloqueados");
					verificarDesbloqueados();
					delay(400);
				}
			}
		});
		threadDesbloqueados.start();
	}
	...
	/**
	 * Retorna la lista de procesadores.
	 *
	 * @return
	 */
	public ArrayList<Ejecucion> getProcesadores() {
		return procesadores;
	}
